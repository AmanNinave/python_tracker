...
def get_all(db: Session = Depends(get_db), user: models.User = Depends(get_current_user) ):
    return task_controller.get_all(db, user.id)

# Get a specific task by ID
@router.get("/{id}", response_model=task_schema.TaskResponse)
def get(id: int, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    return task_controller.get(id, db, user.id)

@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
def delete(id: int, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    return task_controller.delete(id, db, user.id)

@router.put("/{id}", status_code=status.HTTP_202_ACCEPTED)
def update(id: int, updates: task_schema.TaskUpdate, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    return task_controller.update_task(id, updates, db, user.id)
